...
- 保存全局规则前，可实时预览生成的文件名和保存路径。
- 保存设置弹窗按图片格式、文件命名规则和保存目录规则分组显示。
- 设置页面支持中英文切换。
...
可通过设置页顶部的设置按钮选择全局偏好。保存设置弹窗分为 **图片格式**、**文件命名规则** 和 **保存目录规则** 三个标签页，并会实时预览生成文件名和最终保存路径。PNG 使用无损转换；JPG 和 WebP 的质量为 `0.92`；转换为 JPG 时，透明区域会填充为白色。GIF、APNG、动画 WebP 以及浏览器无法转换的图片会保持原文件不变，并显示提示。

可通过设置页顶部的语言切换按钮在中文和英文之间切换。语言选择会保存在浏览器本地。
...
设置页面会为每种文件命名模式显示实时预览。选择自定义模板时，可以点击变量按钮将占位符插入到当前光标位置。模板为空、花括号不成对、变量为空、变量嵌套或使用不支持的变量时，页面会显示本地化的行内错误提示。
...
设置页面会在切换保存目录规则时实时显示路径预览，方便保存前确认目录结构。
